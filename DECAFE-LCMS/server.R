...
"fgsea", "DESeq2", "ggpubr", "stringr", "ggrepel", "UpSetR", "ggdendro", "dendextend","gplots","ROTS")
new_packages <- packages[!(packages %in% installed.packages()[,"Package"])]
...
output$upsetPlot <- renderPlot({
  ...
  upset(df, main.bar.color = '#8ACAAD',
            sets.bar.color = '#8ACAAD', 
            keep.order = TRUE,
            text.scale = 2,
            shade.color = "#A5C2E3", 
...
output$downloadUpsetPlot <- downloadHandler(
      ...
      content = function(file) { 
        ...
        upset_plot = upset(df, main.bar.color = '#8ACAAD',
                              sets.bar.color = '#8ACAAD', 
                              keep.order = TRUE,
                              text.scale = 2, 
                              shade.color = "#A5C2E3", 
...
# filter count and annot by annotationName
  ...
  vstAll <-reactive({
    ...
    removeNotification(notif)
    return(list(normalized_counts=count_normalized, annot_intersect=annot_intersect, count_intersect=count_intersect))
...
output$heatMap <- renderPlot({
  ...
  palette.annot =colorRampPalette(c("#A5C2E3", '#8ACAAD'))
...
# boxplot
 output$bpGeneTarget <- renderPlot({
    ...
    ggplot(res, aes(x=condition,y=count, color=condition)) + geom_boxplot() +
      ...
      stat_summary(fun.y=mean, geom="point", shape=20, size=5, color="#8ACAAD", fill="#8ACAAD")
  })
  ...
  output$histGeneTarget <- renderPlot({
    ...
    ggplot(res, aes(x=as.numeric(count), color=as.factor(condition))) +
      ...
      labs(x = paste0('count normDESq2 of target gene: ',res$genetarget[1]),y = "number of samples") + 
      scale_color_manual(values=c("lightcoral", '#4ab3d6'))
  })
