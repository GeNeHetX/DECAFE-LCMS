...
# Define UI for application that draws a histogram
  ...
  dashboardHeader(
    ...
    title = "DECAFE-LCMS/MS",titleWidth = 300
  ...
  tabItem(tabName ="home",
         ...
         fluidRow(
           column(width = 6,
                  box(width = 12, status = 'info', solidHeader = TRUE, title = h3("Check : Sample_ID and Annotations", icon('file')),
                      withSpinner(DT::DTOutput("tableAnnot"), type = 8, color = "#A5C2E3", size = 1)
                  )
           ),
          column(width = 6,
                  box(width = 12, status = 'info', solidHeader = TRUE, title = h3("Check : Sample_ID and Genes", icon('dna')),
                      withSpinner(DT::DTOutput("counthead"), type = 8, color = "#A5C2E3", size = 1)
                  )
           ),
           ...
           column(width = 12,
                  box(width = 12, status = 'success', solidHeader = TRUE, title = h3("Overview of Condition", icon('chart-simple')),
                      ...
                      withSpinner(plotOutput('upsetPlot'), type = 8, color = "#A5C2E3", size = 1)
                  )
           )
         )
    ),
    tabPanel("Heatmap", height = 1000,
          fluidRow(
        box(class = "map_container",width=12,status='success',title = h2('Heatmap of normalized count',icon('chart-simple')),solidHeader = TRUE,
            column(width=3,
        numericInput("nb_gene_heat", "Number of most variable Gene", min = 10, step = 1, max = 1000,value = 1000),
            ...
            column(width=1),
            column(width=8, 
          withSpinner(plotOutput("heatMap"), type = 8, color = "#A5C2E3", size = 1)
        )
        )),
    ...
    tabPanel("Differential Analysis",
      ...
      box(class = "map_container", width=NULL,status='success',solidHeader=TRUE,title=h1("Volcano Plot",icon('chart-simple')),
        fluidRow(
          ...
          column(width=1),
          column(width=8,
            withSpinner(plotlyOutput("volcano"), type = 8, color = "#A5C2E3", size = 1)
          )
        )
      )
    )),
    ...
    column(width=12,
      box(width=NULL,status='info',title = h1('Table of results',icon('table')),solidHeader = TRUE, 
        withSpinner(DT::dataTableOutput("degTable"), type = 8, color = "#A5C2E3", size = 1)
      )
    )
    )),
    ...
    tabPanel("Boxplot",
      fluidRow(column(width=3,uiOutput('geneTarget')),
        ...
        box(width=12, status = 'success', solidHeader = TRUE, title = h3("Gene Target Histogram", icon('chart-simple')),
          withSpinner(plotOutput('histGeneTarget'), type = 8, color = "#A5C2E3", size = 1)
        )
        ),
        column(width=6,
        box(width=12, status = 'success', solidHeader = TRUE, title = h3("Gene Target Boxplot", icon('chart-simple')),
          withSpinner(plotOutput('bpGeneTarget'), type = 8, color = "#A5C2E3", size = 1)
        )
        )
    ...
    tabPanel("GSEA",
      ...
      fluidRow(
        ...
        withSpinner(DT::dataTableOutput("gsea"), type = 8, color = "#A5C2E3", size = 1)
      ))),
      fluidRow(
      column(width=12,
        box(width=NULL,status='info',title = h1('Tree Pathways',icon('square-poll-horizontal')),solidHeader = TRUE, 
          ...
          withSpinner(plotOutput("treePlot", inline=F, width = 1200, height=1500), type = 8, color = "#A5C2E3", size = 1), style = 'display:block;width:100%;overflow-y: scroll')
      )
      )),id="tabBox",
